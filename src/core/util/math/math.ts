...
export function positionInsideTarget(mouse: PointInterface, target: SceneObjectInterface): boolean {
...
export function positionInsideRotateTarget(mouse: PointInterface, target: SceneObjectInterface): boolean {
    const {x, y} = mouse;
    const {rotate} = target;
